...
function backupconfigs(){
  ...
  do
    ...
    then
      \cp -aufv $config ~/Documents/network/ssh/
    else
      \cp -aufv ~/$config $path_for_bakcup
      fi
    done
  }
...
#mount win
alias win='sudo ntfs-3g /dev/sda2 /mnt/windata;sudo ntfs-3g /dev/nvme0n1p4 /mnt/winos'
...
#----GPU---
if [[ $(which nvidia-smi 2>/dev/null) ]]
then
...
#科学上网
if [[ $(which brook 2>/dev/null) ]]
then
  if [[ -f $(which brook-client 2>/dev/null) ]]
  then
...
#===dev
#alias pipenv_proj='export PIPENV_VENV_IN_PROJECT=1'
#pipenv_proj
